...
void describe('/api/Products', () => {
  ...
  void it('POST new product sanitizes XSS without solving the RESTful XSS challenge', async () => {
    assert.equal(challenges.restfulXssChallenge.solved, false)
    ...
    const res = await request(app)
      ...
      .send({
        name: 'XSS Juice (42ml)',
        description: '<strong>Safe formatting</strong><iframe src="javascript:alert(`xss`)">',
        price: 9999.99,
        image: 'xss3juice.jpg'
      })

    assert.ok(res.headers['content-type']?.includes('application/json'))
    assert.equal(res.body.data.description, '<strong>Safe formatting</strong>')
    assert.equal(challenges.restfulXssChallenge.solved, false)
  })
})
