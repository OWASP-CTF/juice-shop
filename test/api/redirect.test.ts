...
void describe('/redirect', () => {
  ...
  void it('GET rejects redirect requests without a target', async () => {
    const res = await request(app)
      .get('/redirect')
  ...
  void it('GET rejects redirect requests with an unrecognized query parameter', async () => {
    const res = await request(app)
      .get('/redirect?x=y')
    assert.equal(res.status, 406)
    assert.ok(res.headers['content-type']?.includes('text/html'))
  ...
  void it('GET rejects a target that only contains an allowlisted URL', async () => {
    const res = await request(app)
