...
void describe('/redirect', () => {
  ...
  void it('GET no longer redirects to the outdated blockchain.info address passed as "to" parameter', async () => {
    const res = await request(app)
  ...
  void it('GET no longer redirects to the outdated explorer.dash.org address passed as "to" parameter', async () => {
    const res = await request(app)
  ...
  void it('GET no longer redirects to the outdated etherscan.io address passed as "to" parameter', async () => {
    const res = await request(app)
  ...
  void it('GET a clean 406 (no TypeError leakage) when calling /redirect without query parameter', async () => {
    const res = await request(app)
      .get('/redirect')
    assert.equal(res.status, 406)
  ...
  void it('GET a clean 406 (no TypeError leakage) when calling /redirect with an unrecognized query parameter', async () => {
    const res = await request(app)
      .get('/redirect?x=y')
    assert.equal(res.status, 406)
    assert.ok(res.text.includes('Unrecognized target URL for redirect: undefined'))
  ...
  void it('GET is not redirected when an allow-listed URL is only part of the query string (bypass closed)', async () => {
    const res = await request(app)
