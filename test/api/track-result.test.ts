...
void describe('/rest/track-order/:id', () => {
  ...
  void it('GET all orders by injecting into orderId no longer leaks other orders', async () => {
    const res = await request(app)
      .get('/rest/track-order/%27%20%7C%7C%20true%20%7C%7C%20%27')
    assert.equal(res.status, 200)
    assert.ok(res.headers['content-type']?.includes('application/json'))
    assert.ok(Array.isArray(res.body.data))
    // The injection string is now matched literally as an orderId, so it hits nothing.
    // trackOrder then echoes back a single placeholder { orderId } with no real order fields.
