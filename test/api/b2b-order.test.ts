...
void describe('/b2b/v2/orders', () => {
  if (utils.isChallengeEnabled(challenges.rceChallenge) || utils.isChallengeEnabled(challenges.rceOccupyChallenge)) {
    void it('POST rejects an endless loop expression as invalid JSON', async () => {
    ...
    void it('POST rejects a busy-spinning regex expression as invalid JSON', async () => {
      const res = await request(app)
        .post('/b2b/v2/orders')
        .set(authHeader)
        .send({
    ...
    void it('POST rejects a sandbox breakout expression as invalid JSON', async () => {
      const res = await request(app)
        .post('/b2b/v2/orders')
        .set(authHeader)
        .send({
