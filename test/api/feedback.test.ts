...
// The CAPTCHA answer is intentionally no longer part of the /rest/captcha
// response (that was the actual "CAPTCHA Bypass" vulnerability), so tests
// solve the simple arithmetic expression themselves instead of reading it
// straight out of the response body.
const solveCaptcha = (expression: string): string => eval(expression).toString() // eslint-disable-line no-eval
