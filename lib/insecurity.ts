...
/* The key that signs every session token was written into this file, and its public half was
   additionally published under /encryptionkeys. A signing key committed to the source signs a
   valid token for everyone who can read the source - including the role claim - so it is no
   key at all. The pair is minted once per boot and the private half never leaves this module. */
