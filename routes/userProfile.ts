...
export function getUserProfile () {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    // The username is untrusted data, never code: HTML-encode it before it is
    // interpolated into the template so it can neither be evaluated as a Pug
    // expression nor break out into markup.
    ...
    try {
      ...
      // A fixed, restrictive policy: never build the CSP header from user data
      // and never allow eval or inline script.
