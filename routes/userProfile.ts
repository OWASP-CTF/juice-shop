...
export function getUserProfile () {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const CSP = "default-src 'self'; img-src 'self' data:; script-src 'self'; style-src 'self' 'unsafe-inline'; frame-ancestors 'none'"

      res.set({
        'Content-Security-Policy': CSP
      })

      const renderedTemplate = fn(user).replace(/_username_/g, entities.encode(username))
