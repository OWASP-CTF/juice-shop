...
export const promotionVideo = () => {
  return (req: Request, res: Response) => {
    fs.readFile('views/promotionVideo.pug', async function (err, buf) {
      ...
      // The subtitle file lands in the raw text of a script element, where only '<' can end it.
      const encodedSubs = subs.replace(/</g, '&lt;').replace(/>/g, '&gt;')
      res.send(fn().replace('<script id="subtitle"></script>', '<script id="subtitle" type="text/vtt" data-label="English" data-lang="en">' + encodedSubs + '</script>'))
    })
  }
  function favicon () {
