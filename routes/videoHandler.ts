...
export const promotionVideo = () => {
  return (req: Request, res: Response) => {
    fs.readFile('views/promotionVideo.pug', async function (err, buf) {
      ...
      const encodedSubs = Buffer.from(subs, 'utf8').toString('base64')
      compiledTemplate = compiledTemplate.replace('<script id="subtitle"></script>', `<script id="subtitle" type="text/vtt" data-label="English" data-lang="en" data-content="${encodedSubs}"></script>`)
