...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query('SELECT * FROM Users WHERE email = :email AND deletedAt IS NULL', {
      ...
      .then((foundUser) => {
        const user = utils.queryResultToJson(foundUser)
        // A salted digest cannot be matched in the query, so the password is checked here instead.
        const authenticated = user.data?.id != null && security.verifyPassword(req.body.password || '', user.data.password)
        if (authenticated && user.data?.totpSecret !== '') {
          res.status(401).json({
