...
export function getPaymentMethods () {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const cards = await CardModel.findAll({ where: { UserId: user.data.id } })
    cards.forEach(card => {
...
export function getPaymentMethodById () {
  return async (req: Request, res: Response, next: NextFunction) => {
    const user = security.authenticatedUsers.from(req)
    if (!user?.data?.id) return res.status(401).json({ status: 'error', data: 'Unauthorized' })
    const card = await CardModel.findOne({ where: { id: req.params.id, UserId: user.data.id } })
    const displayableCard: displayCard = {
      UserId: 0,
      id: 0,
      fullName: '',
...
export function delPaymentMethodById () {
  return async (req: Request, res: Response, next: NextFunction) => {
    const user = security.authenticatedUsers.from(req)
    if (!user?.data?.id) return res.status(401).json({ status: 'error', data: 'Unauthorized' })
    const card = await CardModel.destroy({ where: { id: req.params.id, UserId: user.data.id } })
    if (card) {
