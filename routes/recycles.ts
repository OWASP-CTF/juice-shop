...
export const verifyRecycleAddress = () => async (req: Request, res: Response, next: NextFunction) => {
  const addressId = Number(req.body.AddressId)
  if (!Number.isInteger(addressId)) {
    return res.status(400).json({ error: 'A valid address is required.' })
  }
  const ownedAddress = await AddressModel.findOne({ where: { id: addressId, UserId: req.body.UserId } })
