...
export function saveLoginIp () {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (loggedInUser !== undefined) {
      ...
      // Always sanitize the attacker-controlled true-client-ip header before persisting
      // it as lastLoginIp, so no raw HTML/script payload is ever stored (stored XSS).
