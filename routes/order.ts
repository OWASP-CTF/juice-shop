  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    BasketModel.findOne({ where: { id }, include: [{ model: ProductModel, paranoid: false, as: 'Products' }] })
      .then(async (basket: BasketModel | null) => {
        if (basket != null) {
          // Withdrawn products are only soft deleted, so the join still hands them back. An order
          // still holding one is refused outright rather than billed without telling the customer.
          const orderedIds = [...new Set((basket.Products ?? []).map(({ id }) => id))]
