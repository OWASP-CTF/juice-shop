  ...
  return async (req: Request, res: Response) => {
    const chatTools = {
      ...
      generateCoupon: tool({
        ...
        inputSchema: z.object({
          discount: z.number().int().min(0).max(10).describe('The discount percentage for the coupon (maximum 10)') // vuln-code-snippet vuln-line chatbotPromptInjectionChallenge chatbotGreedyInjectionChallenge
        }),
        execute: async ({ discount }) => {
          ...
          if (discount > 10) {
            return { error: 'Discount exceeds the maximum allowed value' }
          }
