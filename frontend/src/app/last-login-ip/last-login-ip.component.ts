  ...
  parseAuthToken () {
    ...
    if (token) {
      ...
      if (payload.data.lastLoginIp) {
        // The IP comes straight from a client-controlled request header, so it is data, not
        // markup. Marking it as trusted HTML let whatever the header carried render as HTML.
