...
const routes: Routes = [
  ...
  { // vuln-code-snippet neutral-line web3SandboxChallenge
    /* The sandbox compiles and runs smart contract code on demand. It is internal tooling, so it
       is gated on the administrator role like the other internal screens, instead of relying on
  ...
  { // vuln-code-snippet neutral-line tokenSaleChallenge
    /* The sale has not been announced. Its page now lives at an ordinary path behind an actual
       authorisation check, instead of at an address reconstructed at runtime by helpers that ship
       in the same bundle - obfuscation conceals, it does not authorise. */
