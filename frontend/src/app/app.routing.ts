...
const routes: Routes = [
  ...
  { // vuln-code-snippet neutral-line tokenSaleChallenge
    // An ordinary path and a role check. The page used to be reachable only through
    // tokenMatcher, which rebuilt a secret address from two character-arithmetic helpers so it
    // could not be read off the route table - obscurity standing in for a permission check.
