...
/* The token sale used to be reachable through a path assembled at runtime out of character
   arithmetic, so the route existed but its address was meant to be unguessable. That is
   concealment, not access control: anyone who read the bundle recovered the path. The screen now
   sits on a plain, honest address and is protected by the same role check as the other
   restricted screens, which is what actually keeps unauthorised visitors out. */
