...
const routes: Routes = [
  ...
  /* The web3 code sandbox is a development tool that was deployed to the shop by accident. It
     compiles and runs smart contract code on demand and was reachable by anyone who knew the
     path - and an unlinked path is not a restriction, it is a guess away. It is not part of the
     shop, so it is not routed from the shop. */
  {
    path: 'chatbot',
...
/* The token sale has not been announced, so the shop does not ship a route to it. It used to be
   registered behind tokenMatcher(), which de-obfuscated the path with the token1()/token2()
   helpers - and those helpers travelled to every visitor in the bundle, so the "secret" URL was
   one console call away. Obfuscation is not access control. A page that is still confidential is
   simply not part of the released application; it is added when the sale is announced. */
