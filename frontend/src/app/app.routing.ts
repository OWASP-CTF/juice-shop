...
const routes: Routes = [
  ...
  { // vuln-code-snippet neutral-line tokenSaleChallenge
    // The sale page keeps its route, at an ordinary path. It used to be reachable only through
    // tokenMatcher, which rebuilt a secret address from two character-arithmetic helpers so the
    // URL could not be read off the route table - obscurity standing in for a permission check.
    // Anyone who ran the helpers had the address, and having it was the whole control.
