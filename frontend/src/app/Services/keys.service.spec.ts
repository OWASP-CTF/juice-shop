...
describe('KeysService', () => {
  ...
  it('should post walletAddressSend directly to the rest api', () => {
    service.walletAddressSend('walletAddress', 'nonce', 'signature').subscribe((res) => {
      expect(res).toBe('apiResponse')
    })
    const req = httpMock.expectOne('http://localhost:3000/rest/web3/walletExploitAddress')
    expect(req.request.method).toBe('POST')
    expect(req.request.body).toEqual({ walletAddress: 'walletAddress', nonce: 'nonce', signature: 'signature' })
    req.flush('apiResponse')
  })
  ...
  it('should handle error in walletAddressSend', () => {
    let capturedError: any
    service.walletAddressSend('walletAddress', 'nonce', 'signature').subscribe({ next: () => expect(true).toBe(false), error: (e) => { capturedError = e } })
    const req = httpMock.expectOne('http://localhost:3000/rest/web3/walletExploitAddress')
    req.error(new ErrorEvent('Request failed'), { status: 500, statusText: 'Internal Server Error' })
    expect(capturedError.status).toBe(500)
  })
