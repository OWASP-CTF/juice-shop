...
export class TrackResultComponent implements OnInit {
  ...
  ngOnInit (): void {
    ...
    this.trackOrderService.find(this.orderId).subscribe((results) => {
      // The order number is reflected back from the request, so it is rendered as text
      // via interpolation in the template and must never be treated as trusted HTML.
