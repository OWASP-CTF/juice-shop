...
export class TrackResultComponent implements OnInit {
  ...
  ngOnInit (): void {
    ...
    this.trackOrderService.find(this.orderId).subscribe((results) => {
      // The order id is echoed back verbatim from the query string, so it must be treated
      // as plain data. Marking it as trusted HTML turned this page into a reflected XSS sink.
