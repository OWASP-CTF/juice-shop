...
describe('TrackResultComponent', () => {
    ...
    it('should bind the order number as a plain string so Angular sanitizes it', () => {
        component.orderId = '<a src="link">Link</a>'
        trackOrderService.find.mockReturnValue(of({ data: [{ orderId: component.orderId }] }))
        component.ngOnInit()

        // A plain string, not a bypassed SafeHtml value, so Angular's own sanitizer runs on it.
        expect(component.results.orderNo).toBe('<code><a src="link">Link</a></code>')
        expect(typeof component.results.orderNo).toBe('string')
    })
