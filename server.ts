...
function configureApp (app: ReturnType<typeof express>, seq: typeof sequelize) {
  ...
  /* Anyone may leave feedback, signed in or not, which is what the form has always offered.
     Requiring a session here removed that; ownership is still decided on the server just below,
     so an anonymous entry is recorded with no user attached rather than being refused. */
