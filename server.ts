...
function configureApp (app: ReturnType<typeof express>, seq: typeof sequelize) {
  ...
  /* Users: Only POST is allowed in order to register a new user. Reading user records is
     administrative data and requires the admin role, not merely a valid token. */
  ...
  /* Registration must never let the client pick its own role. Stripping the attribute
     here means the model's 'customer' default applies to the persisted row. */
  app.post('/api/Users', (req: Request, res: Response, next: NextFunction) => {
